...
from training.train0 import train_one_epoch, evaluate
...
def main():
    ...
    if is_master(args, local=args.log_local):
        log_base_path = os.path.join(args.output, args.name)
    ...
    if is_master(args):
        args.tensorboard_path = os.path.join(
            args.output, args.name, "tensorboard") if args.tensorboard else ''
        args.checkpoint_path = os.path.join(
            args.output, args.name, "checkpoints")
    ...
    if is_master(args):
        ...
        logging.info("Params:")
        params_file = os.path.join(args.output, args.name, "params.txt")
            ...
            if len(ckpts_list) > 0:
                ...
                for epoch, it in ckpts_list:
                    checkpoint_fname = os.path.join(
                        args.checkpoint_path, f"epoch_{epoch}_iter_{it}.bin")
                    try:
                        # check valid
        ...
        if os.path.isfile(args.resume):
            ...
            if args.prune_image and args.prune_text:
                ...
                if args.load_last_stage is False:
                    ...
                    args.save_logs = args.output and args.output.lower() != 'none' and is_master(args)
            else:
                sd = checkpoint["state_dict"]
                new_state_dict = {}
...
def copy_codebase(args):
    ...
    new_code_path = os.path.join(args.output, args.name, "code")

...
labels = cifar_10_test.features['label'].names  # Extract class labels
label_id_dict = dict(zip(labels, range(len(labels))))  # Map labels to IDs
id_label_dict = dict(zip(range(len(labels)), labels))  # Map IDs to labels

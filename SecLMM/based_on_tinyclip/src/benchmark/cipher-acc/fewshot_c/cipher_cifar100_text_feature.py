...
import spu.spu_pb2 as spu_pb2  # For SP极U protocol buffer definitions
...
def classifier(input_ids, attention_mask, pixel_values极, params):
    config = CLIPConfig()  # Initialize CLIP configuration
    model = FlaxCLIPModel(config=config)  # Initialize CLIP model

    # Get model outputs
    outputs = model.get_text_features(input_ids=input_ids, attention_mask=attention_mask,  params=params)
...
np.savetxt("outputs_text_cifar100.txt", outputs_ids, fmt="%.8f")
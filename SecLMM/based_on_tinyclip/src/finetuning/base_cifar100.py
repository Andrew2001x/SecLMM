...
# Load dataset
cifar_10 = load_dataset('cifar100')
labels = cifar_10['train'].features['fine_label'].names
...
def f_train(cifar_10, sh=5):
    f_samples = []
    for label in labels:
        samples = cifar_10['train'].filter(lambda x: x['fine_label'] == label_id_dict[label]).select(range(sh))
...
def f_eval(clf, scaler, cifar_10_test, batch_size=200):
    # 提取测试图像和标签
    images = [sample['img'] for sample in cifar_10_test]
    test_labels = jnp.array([sample['fine_label'] for sample in cifar_10_test])  # 使用 JAX 数组
...
acc, pred_labels = f_eval(loaded_clf, loaded_scaler, cifar_10['test'])

# Print results
print(f"F accuracy (sh={sh}): {acc:.4f}")
print(classification_report(cifar_10['test']['fine_label'], pred_labels, target_names=labels, digits=4))

...
labels = cifar_10['train'].features['gender'].names
...
def f_train(cifar_10, sh=5):
    f_samples = []
    for label in labels:
        samples = cifar_10['train'].filter(lambda x: x['gender'] == label_id_dict[label]).select(range(sh))
        f_samples.extend(samples)
    
    # Extract features and labels
    images = [sample['image'] for sample in f_samples]
    sample_labels = [sample['gender'] for sample in f_samples]
...
def f_eval(clf, scaler, cifar_10_test, batch_size=200):
    ...
    test_labels = jnp.array([sample['gender'] for sample in cifar_10_test])  # 使用 JAX 数组
...
model_filename = "f_clf_model_base_fairgender.pkl"
joblib.dump(clf, model_filename)
scaler_filename = "f_scaler_base_fairgender.pkl"
...
print(classification_report(cifar_10['validation']['gender'], pred_labels, target_names=labels, digits=4))
